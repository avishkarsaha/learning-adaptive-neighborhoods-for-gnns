...
def gcn_norm(
    ...
    if isinstance(edge_index, SparseTensor):
        ...
        if not adj_t.has_value():
            adj_t.fill_value(1.0, dtype=dtype)
        if add_self_loops:
...
class GCNIIdenseConv(MessagePassing):
    ...
    def forward(
        self, x: Tensor, edge_index: Adj, alpha, h0, beta, edge_weight: OptTensor = None
    ) -> Tensor:
        """ """
        ...
        out = (
            self.propagate(edge_index, x=support, edge_weight=edge_weight, size=None)
            + initial
        )
        return out
