...
def plot2():
    ...
    # load csv
    data = np.loadtxt(open('dgg_noisy_results.csv', 'rb'), delimiter=',')

    original = data[::2, :]
    dgg = data[1::2, :]

    models = [original, dgg]
    models_by_dataset = [np.split(data, 3) for data in models]

    backbones = ['GCN', 'GCNII']
    datasets = ['Cora', 'Citeseer', 'Flickr']

    colors = ['#fb5607', '#8338ec']
    ...
    for idx_data, ax in enumerate(axes):
        ...
        for idx_bb in range(len(backbones)):
            ...
            ax.set_ylabel('Accuracy')
            ax.set_xlabel('Noisy edges (%)')
            ax.set_xticks([0, 1, 2, 3, 4],[0, 50, 100, 150, 200])
