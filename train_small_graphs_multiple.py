"""
This training script is for training a GCN and a GCN_DGG simultaneously, and using
the adjacency generated by the GCN_DGG in the GCN.

Effectively we have taken the GCN_DGG's multi-object loss of L = a*loss + b*loss
and split it into training one model for each loss.
"""
...
def train_gcn(args, model, optimizer, data, device, epoch, writer):
    model.train()
...
def test(model_gcn, model_gcn_dgg, data, device):
    ...
    with torch.no_grad():
        ...
        acc_test, loss_test, out_adj = cross_infer1(
            adj, data, device, features, labels, model_gcn, model_gcn_dgg
        )
...
def cross_infer1(adj, data, device, features, labels, model_gcn, model_gcn_dgg):
    _, out_adj, _ = model_gcn_dgg(features, adj, edge_index=data.edge_index,)
    output, _, _ = model_gcn(features, out_adj, edge_index=data.edge_index,)
...
def cross_infer2(adj, data, device, features, labels, model_gcn):
    output, _, _ = model_gcn(features, adj, edge_index=data.edge_index,)
    acc_test = accuracy(output[data.test_mask], labels[data.test_mask].to(device))
    loss_test = F.nll_loss(output[data.test_mask], labels[data.test_mask].to(device))
